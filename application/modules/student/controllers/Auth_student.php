...
class Auth_student extends CI_Controller {
    ...
    function login() {
        ...
        if ($_POST AND $this->form_validation->run() == TRUE) {
            ...
            if (count($student) > 0) {
                ...
                $this->session->set_userdata('uid_student', $student[0]['mahasiswa_id']);
                $this->session->set_userdata('unis_student', $student[0]['mahasiswa_nim']);
                $this->session->set_userdata('ufullname_student', $student[0]['mahasiswa_full_name']);
                $this->session->set_userdata('mahasiswa_img', $student[0]['mahasiswa_img']);
                if ($location != '') {
                ...
                if ($location != '') {
                    $this->session->set_flashdata('failed', 'Maaf, NIM dan password tidak cocok!');
    ...
    function logout() {
        ...
        $this->session->unset_userdata('mahasiswa_img');
