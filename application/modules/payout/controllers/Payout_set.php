...
// Siswa
    if (isset($f['r']) && !empty($f['r']) && $f['r'] != '') {
      $params['mahasiswa_nim'] = $f['r'];
      $param['mahasiswa_nim'] = $f['r'];
      $cashback['mahasiswa_nim'] = $f['r'];
      $logs['mahasiswa_nim'] = $f['r'];
      $siswa = $this->Student_model->get(array('mahasiswa_nim'=>$f['r']));
    }
    ...
    $param['status'] = 1;
    $cashback['status'] = 1;
    $pay['mahasiswa_id']=$siswa['mahasiswa_id'];
    $cashback['mahasiswa_id']=$siswa['mahasiswa_id'];
    $logs['mahasiswa_id']=$siswa['mahasiswa_id'];
    ...
    $data['siswa'] = $this->Student_model->get(array('mahasiswa_id'=>$siswa['mahasiswa_id'], 'group'=>TRUE));
    $data['student'] = $this->Bulan_model->get($pay);
    $data['bulan'] = $this->Bulan_model->get(array('mahasiswa_id'=>$siswa['mahasiswa_id']));
...
// Siswa
    ...
    $data = pdf_create($html, $siswa['mahasiswa_full_name'], TRUE, 'A4', TRUE);
  }
...
// Siswa
    ...
    $pay['paymentt'] = TRUE;
    $param['status'] = 1;
    $param['mahasiswa_id']=$siswa['mahasiswa_id'];
    $cashback['status'] = 1;
    $pay['mahasiswa_id']=$siswa['mahasiswa_id'];
    $cashback['mahasiswa_id']=$siswa['mahasiswa_id'];
    ...
    $data = pdf_create($html, 'Cetak_Struk_'.$siswa['mahasiswa_full_name'].'_'.date('Y-m-d'), TRUE, 'A4', TRUE);
  }
...
// View data detail
  ...
  public function payout_bulan($id = NULL, $mahasiswa_id = NULL) {

    if ($id == NULL AND $mahasiswa_id == NULL OR $mahasiswa_id == NULL) {
      redirect('manage/payout');
    }

    $data['class'] = $this->Student_model->get_class();
    $data['payment'] = $this->Payment_model->get(array('id' => $id));
    $data['bulan'] = $this->Bulan_model->get(array('payment_id' => $id, 'mahasiswa_id' => $mahasiswa_id));
    $data['in'] = $this->Bulan_model->get_total(array('status'=>1, 'payment_id' => $id, 'mahasiswa_id' => $mahasiswa_id));
    $data['student'] = $this->Student_model->get(array('id'=> $mahasiswa_id));
  ...
  public function payout_bebas($id = NULL, $mahasiswa_id = NULL, $bebas_id = NULL, $pay_id =NULL) {

    // if ($id == NULL AND $mahasiswa_id == NULL AND $bebas_id == NULL OR $bebas_id == NULL) {
    //   redirect('manage/payout');
    // }
    if ($_POST == TRUE) {
      ...
      $data['bebas'] = $this->Bebas_model->get(array('payment_id' => $this->input->post('payment_payment_id'), 'mahasiswa_nim' => $this->input->post('mahasiswa_nim')));
      ...
      if ($this->input->post('bebas_pay_bill') > $sisa OR $this->input->post('bebas_pay_bill') == 0) {
        ...
        redirect('manage/payout?n='.$student['period_period_id'].'&r='.$student['mahasiswa_nim']);
      } else {
        ...
        $log = array(
          ...
          'student_mahasiswa_id' => $this->input->post('student_mahasiswa_id'),
      ...
      if ($activated['setting_value'] == 'Y') {
        ...
        $message = "Pembayaran ".$student['pos_name'].' - T.A '.$student['period_start'].'/'.$student['period_end'].'-'.$this->input->post('bebas_pay_desc').' a/n '.$student['mahasiswa_full_name'].' Berhasil';
      ...
      $data['class'] = $this->Student_model->get_class();
      $data['payment'] = $this->Payment_model->get(array('id' => $id));
      $data['bebas'] = $this->Bebas_model->get(array('payment_id' => $id, 'mahasiswa_id' => $mahasiswa_id));
      $data['student'] = $this->Student_model->get(array('id'=> $mahasiswa_id));
      $data['bill'] = $this->Bebas_pay_model->get(array('bebas_id'=>$bebas_id, 'mahasiswa_id'=>$mahasiswa_id, 'payment_id'=>$id));
  ...
  function pay($payment_id = NULL, $mahasiswa_id =NULL, $id = NULL) { 

    $lastletter = $this->Letter_model->get(array('limit' => 1));
    $student = $this->Bulan_model->get(array('mahasiswa_id'=>$mahasiswa_id,'id'=>$id));
    $user = $this->Setting_model->get(array('id' => 8));
    $password = $this->Setting_model->get(array('id' => 9));
    $activated = $this->Setting_model->get(array('id' => 10));
    ...
    $log = array(
      'bulan_bulan_id' => $id,
      'student_mahasiswa_id' => $mahasiswa_id,
    ...
    if ($activated['setting_value'] == 'Y') {
      ...
      $userkey = $user['setting_value']; 
      $passkey = $password['setting_value']; 
      $telepon = $student['mahasiswa_parent_phone'];
      ...
      $message = "Pembayaran ".$namePay.' - ('.$student['month_name'].' '. $mont.') a/n '.$student['mahasiswa_full_name'].' Berhasil';
  ...
  function not_pay($payment_id = NULL, $mahasiswa_id =NULL, $id = NULL) { 
    $student = $this->Bulan_model->get(array('mahasiswa_id'=>$mahasiswa_id,'id'=>$id));
    $pay = array(
      'bulan_id' => $id,
  ...
  function printPay($payment_id = NULL, $mahasiswa_id =NULL, $id = NULL) {
    $this->load->helper(array('dompdf'));
    $this->load->helper(array('tanggal'));

    if ($id == NULL)
      redirect('manage/payout/payout_bulan/'.$payment_id.'/'.$mahasiswa_id);
    ...
    $data = pdf_create($html, $data['printpay']['mahasiswa_full_name'], TRUE, 'A4', TRUE);
  }

  function printPayFree($payment_id = NULL, $mahasiswa_id =NULL, $id = NULL) {
    $this->load->helper(array('dompdf'));
    $this->load->helper(array('tanggal'));

    if ($id == NULL)
      redirect('manage/payout/payout_bebas/'.$payment_id.'/'.$mahasiswa_id);
    ...
    $data['setting_district'] = $this->Setting_model->get(array('id' => SCHOOL_DISTRICT));
    $data['setting_city'] = $this->Setting_model->get(array('id' => SCHOOL_CITY));  
    $data['bebas'] = $this->Bebas_model->get(array('payment_id' => $payment_id, 'mahasiswa_id' => $mahasiswa_id));
  ...
  function delete_pay_free($payment_id = NULL, $mahasiswa_id =NULL, $bebas_id = NULL, $id =NULL) {
    ...
    if ($this->input->is_ajax_request()) {
      ...
      redirect('manage/payout/payout_bebas/' . $payment_id.'/'.$mahasiswa_id.'/'.$bebas_id);
    }
