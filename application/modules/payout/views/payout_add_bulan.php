			...
			<div class="col-md-4"> 
				<div class="box box-info">
					...
					<div class="box-footer">
						<a href="<?php echo site_url('manage/payout?'.'n='.$payment['period_period_id'].'&'.'r='.$student['mahasiswa_nim']) ?>" class="btn btn-primary pull-right">Kembali</a>
					</div>
				</div>
			</div>
			<div class="col-md-8">
							...
							<table class="table table-striped table-hover">
								...
								<tbody>
									<tr ng-repeat="item in payouts" ng-style="item.bulan_status == 1 ? {'color' : '#00E640'} : {'color' : 'red'}">	
										...
										<td>
											<a class="btn btn-success btn-xs" ng-class="item.bulan_status == 0 ? 'disabled': 'view-pdf'" target="_blank" title="Cetak Slip" href="<?php echo site_url('manage/payout/printPay/') ?>{{item.payment_payment_id}}/{{item.student_mahasiswa_id}}/{{item.bulan_id}}"><span class="fa fa-print"></span> Cetak</a>
										</td>
									</tr>
								</tbody>
							</table>
...
<script>
	...
	var mahasiswa_id = <?php echo $student['mahasiswa_id'] ?>;
	...
	App.controller('payoutCtrl', function($scope, $http) {
		...
		$scope.getPayout = function() {
			...
			var url = SITEURL + 'api/get_payout_bulan/'+payment_id+'/'+mahasiswa_id;
		...
		$scope.pay = function(bulan_id) {
			$.ajax({
				method: "Get",
				url: SITEURL + "manage/payout/pay/"+payment_id+'/'+mahasiswa_id+'/'+bulan_id,
		...
		$scope.notPay = function(bulan_id) {
			$.ajax({
				method: "Get",
				url: SITEURL + "manage/payout/not_pay/"+payment_id+'/'+mahasiswa_id+'/'+bulan_id,
				success: function(data) {
					$scope.getPayout();
				}
			});
